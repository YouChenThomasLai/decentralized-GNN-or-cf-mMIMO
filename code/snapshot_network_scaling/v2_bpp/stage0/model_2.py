...
class RIS_readout_AP(nn.Module):
    def __init__(self,M,N,L,Pt,in_dim, num_users):
        ...
        self.fe_AP = nn.Linear(L*num_users,self.N*2)
...
class node_update(nn.Module):
    def __init__(self,M,N,L,D,Pmax,b,ch,AP, device, num_users=8):
        ...
        for BS_idx in range(self.AP):
            ...
            RIS_readout_AP_list.append(
                RIS_readout_AP(M,N,L,Pmax,ch*(D+1),num_users)
            )
...
if __name__ == '__main__':
    ...
    model = node_update(M, N, L, 3, Pmax, 2, 4, 5, device, K).to(device)
    W, theta = model(user_feature, e, user_index, e_dir)
    loss = dataloader.compute_loss(W, theta, Pmax, device)
    print(loss)
